...
def run():
    ...
    team_name = "hanyolo"
    password = "nfspwHJOiWDS"
    evaluation_server_url = "http://teknofest.cezerirobot.com:1025/"
    ...
    plt.plot(detection_model.yliste, detection_model.xliste, marker='o', linestyle='-', color='b')
    plt.title('X vs Y')
    plt.xlabel('Y')
    plt.ylabel('X')
    plt.grid(True)
    plt.show()
    plt.figure(figsize=(10, 6))
    plt.plot(detection_model.realy, detection_model.realx, marker='o', linestyle='-', color='r')
    plt.title('Real x vs Real Y')
    plt.xlabel('Y')
    plt.ylabel('X')
    plt.grid(True)
    plt.show()

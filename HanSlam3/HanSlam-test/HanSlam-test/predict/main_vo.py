    ...
    def path_tracker(self, image_path):
        try:
            ...
            if x is None:
                ...
                print(self.TRANSLATION_Y)
                self.lastx=x[0] + float(self.TRANSLATION_Y)
                self.lasty=y[0]+ float(self.TRANSLATION_X)
                self.lastz=z[0]
                return {"x": self.lastx*-1, "y": self.lasty*-1, "z": self.lastz}
